...
export default function CallModal() {
    ...
    return (<div className="call_modal">
        <div className="call_box" style={{
            display: (answer && call.video) ? 'none' : 'flex'
        }} >

            <Box sx={{ padding: '20px 0', textAlign: 'center' }} >
                <Avatar src={call.avatar} sx={{ width: '86px', height: '86px', m: 'auto' }} />
                <Typography variant='h4'>{call.username}</Typography>
                <Typography variant='h6'>{call.fullname}</Typography>
                {answer
                    ? <div>
            ...
            <div className="call_menu">
                <button className="material-icons" style={{ color: 'red' }}
                    onClick={handleEndCall}>
                    call_end
                </button>
                {(call.recipient === auth.user._id && !answer) && <>
                    {call.video
                        ? <button className="material-icons" style={{ color: 'green' }}
